...
type ColumnType = ColumnDef<PerformanceDataInterface, unknown>;

interface PerformanceDataListPageProps {
...
export function PerformanceDataListPage(props: PerformanceDataListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PerformanceDataInterface>>(
    () => `/performance-data?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: PerformanceDataInterface) => {
    if (hasAccess('performance_data', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/performance-data/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'week', header: 'week', accessorKey: 'week' },
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/performance-data/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('performance_data', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/performance-data/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="performance_data" />
      <Flex justifyContent="space-between" mb={4}>
        ...
        {hasAccess('performance_data', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/performance-data/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
